...
**Google 账号登录：** 集成 NextAuth v5 + Google OAuth，用户需登录后才能使用代码生成功能。登录后右上角展示用户头像和信息，API 路由同步受到鉴权保护。
...
- **身份认证**: NextAuth v5 (Auth.js) + Google OAuth
- **图标**: Lucide React
...
- Google OAuth 凭证（[Google Cloud Console](https://console.cloud.google.com/apis/credentials) 创建）
...
> **Google OAuth 配置说明：** 在 [Google Cloud Console](https://console.cloud.google.com/apis/credentials) 创建 OAuth 2.0 客户端，应用类型选"Web 应用"，将 `http://localhost:3000/api/auth/callback/google` 添加为已授权的重定向 URI。
...
```
ai-code-forge/
├── auth.ts                        # NextAuth v5 配置（Google OAuth）
├── middleware.ts                   # API 路由鉴权中间件
├── instrumentation.ts             # 全局代理设置（国内网络）
├── src/
│   ├── app/
│   │   ├── api/
...
│   │   └── page.tsx                # 主页面（含登录门控）
│   ├── components/
│   │   ├── AuthProvider.tsx        # NextAuth SessionProvider 封装
│   │   ├── Header.tsx              # 顶部导航（含用户头像/登录按钮）
